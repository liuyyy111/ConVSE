...
def build_vocab(data_path, data_name, caption_file, threshold):
    ...
    uncommon_words = [word for word, cnt in counter.items() if cnt < 4]
    ...
    for i, word in enumerate(words):
        vocab.add_word(word)
    return vocab, uncommon_words


def main(data_path, data_name):
    vocab, uncommon_words = build_vocab(data_path, data_name, caption_file=annotations, threshold=4)
